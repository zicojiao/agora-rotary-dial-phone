...
try {
  const ticket = issueCallTicket('rotary_test', '42');
  ...
  assert.equal(isCallTicketFor(ticket, 'rotary_test', '42'), true);
  assert.equal(isCallTicketFor(ticket, 'someone_elses_channel', '42'), false);
  assert.equal(isCallTicketFor(ticket, 'rotary_test', '43'), false);
  ...
  assert.throws(
    () => issueCallTicket('rotary_test', '42'),
    /CALL_TICKET_SECRET/,
  );
